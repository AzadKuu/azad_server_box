...
/// On HarmonyOS (ohos), the native assets manifest is not available to the
/// Dart VM. We manually load libsqlite3mc.so with RTLD_GLOBAL so that its
/// symbols enter the process-wide symbol table and `@ffi.Native` process
/// lookup can resolve them.
bool _ensureOhosLibraryLoaded() {
  if (Platform.operatingSystem == 'ohos') {
    try {
      // RTLD_LAZY | RTLD_GLOBAL = 1 | 0x100 = 257
      _dlopen('libsqlite3mc.so'.toNativeUtf8().cast(), 257);
    } catch (_) {
      // Ignore – the library may already be loaded or will be found later.
    }
  }
  return true;
}
...
final supportsPrepareV3 = _ensureOhosLibraryLoaded() && sqlite3_libversion_number() >= _firstVersionForV3;
